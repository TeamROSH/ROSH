...
{	
	...
	decoded_string = (char*)kmalloc(encoded_length / 4 * 3); 

	for(int i = 0, j = 0; i < encoded_length; i+= 4, j+=3)
	{
		// going through decoded string and converting to ascii
		curr_dword = decoded_string[encoded_string[i] - SHIFT_B64];
		curr_dword = (curr_dword << 6) | decoded_string[encoded_string[i + 1] - SHIFT_B64];
		curr_dword = encoded_string[i + 3] == '=' ? 0 &  MAX_CHAR: (curr_dword << 6) | decoded_string[encoded_string[i + 2] - SHIFT_B64];
		curr_dword = encoded_string[i + 3] == '=' ? 0 & MAX_CHAR : (curr_dword << 6) | decoded_string[encoded_string[i + 3] - SHIFT_B64];
